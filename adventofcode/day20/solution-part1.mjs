        ...
        for (let j=0; j<bordersArr.length; j++) {
            if (j !== currentTileIdx) {
                ...
                if (currentBorderToCompare.includes(currentBorderReversed) || currentBorderToCompare.includes(currentBorder)) {
                    counter ++;
                    // console.log(`border ${currentBorder} found at ${j}`);
                }
            }
        }
    }
...
let bordersArr = giveBordersArr(puzzleInput);
...
for (let i=0; i<4; i++) {
    product = product * cornerIdxs[i];
}

console.log(`the final answer is: ${product}`);