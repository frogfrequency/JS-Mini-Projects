...
let testArrClean = [];
let testString = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd byr:1937 iyr:2017 cid:147 hgt:183cm`;


function returnPreprocessedArr(arr){
    let output = [];
...
function returnCountOfValids(input) {
    let masterValidCounter = 0;
    let arr = returnPreprocessedArr(input); // purifies the dirty inputArr
    for (let i=0; i<arr.length; i++) {
        let currentElement = arr[i];
        let byr = returnPropertyValue('byr:', currentElement);
        let iyr = returnPropertyValue('iyr:', currentElement);
        let eyr = returnPropertyValue('eyr:', currentElement);
        let hgt = returnPropertyValue('hgt:', currentElement);
        let hcl = returnPropertyValue('hcl:', currentElement);
        let ecl = returnPropertyValue('ecl:', currentElement);
        let pid = returnPropertyValue('pid:', currentElement);
        let byrIsValid = returnBirthYearValidity(byr);
        let iyrIsValid = returnIssueYearValidity(iyr);
        let eyrIsValid = returnExpirationYearValidity(eyr);
        let hgtIsValid = returnHeightValidity(hgt);
        let hclIsValid = returnHairColorValidity(hcl);
        let eclIsValid = returnEyeColorValidity(ecl);
        let pidIsValid = returnPassportIdValidity(pid);
        // console.log(`byr: ${byr}(${byrIsValid}), iyr: ${iyr}(${iyrIsValid}), eyr: ${eyr}(${eyrIsValid}), hgt: ${hgt}(${hgtIsValid}), hcl: ${hcl}(${hclIsValid}), ecl: ${ecl}(${eclIsValid}), pid: ${pid}(${pidIsValid})`);
        if (byrIsValid && iyrIsValid && eyrIsValid && hgtIsValid && hclIsValid && eclIsValid && pidIsValid) {
            masterValidCounter++
        }
    }
    console.log(masterValidCounter);
}

function returnPropertyValue(property, string) {
    let indexOfProperty = string.indexOf(property);
    if (indexOfProperty === -1) {
        return undefined
    }
    let indexSpaceAfterProperty = string.indexOf(' ', indexOfProperty)
    let outputValue = string.slice(indexOfProperty+4, indexSpaceAfterProperty);
    return outputValue;
}

// property-checks: 

function returnBirthYearValidity(value) {
...
function returnIssueYearValidity(value) {
...
function returnExpirationYearValidity(value) {
    if (value <= 2030 && 2020 <= value) {
        return true
    } else {
        return false
    }
...
function returnHeightValidity(value) {
    if (value !== undefined) {
        let number = value.slice(0, value.length-2)
        let unit = value.slice(value.length-2, value.length); 
        if (unit === 'cm') {
...
function returnHairColorValidity(value) {
    if (value !== undefined) {
        if (value.charAt(0) === '#') {
            let valueWithoutFirstChar = value.slice(1, value.length)
            if (valueWithoutFirstChar.length === 6) {
                for (let i=0; i<valueWithoutFirstChar.length; i++) {
                    let currentCharacter = valueWithoutFirstChar[i];
                    if (currentCharacter.match(/[0-9a-f]/) !== null) {
                        return true
                    }
...
function returnEyeColorValidity(value) {
    let validColors = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'];
    for (let i=0; i<validColors.length; i++) {
        if (validColors[i] === value) {
            return true
        }
    }
    return false
...
function returnPassportIdValidity(value) {
    if (value !== undefined) {
        if (value.length === 9) {
            if (0 <= value && value <= 999999999) {
                return true
            }
        }
    }
