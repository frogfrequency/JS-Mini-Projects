...
function replaceNewLineWithSpace(arr){
    let output = [];
...
function returnCountOfValids(input) {
    ...
    let arr = replaceNewLineWithSpace(input); // purifies the dirty inputarr
    for (let i=0; i<arr.length; i++) {
        let currentPassportValidCounter = 0;
        let requiredSubstrings = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];
        for (let j=0 ; j<requiredSubstrings.length; j++) {
            let currentElement = arr[i];
            let currentSubstring = requiredSubstrings[j];
            if (currentElement.includes(currentSubstring)) {
                currentPassportValidCounter++
            }
        }
        if (currentPassportValidCounter === 7) {
            masterValidCounter++
        }
    }
